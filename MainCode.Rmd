...
avgtemp <- df_long %>% group_by(Time,Tag,`Anzahl Raynaud Attacken`) %>% summarise(avgtemp = mean(Temperature, na.rm = TRUE))
...
NoAttacks  %>%
  ggplot(aes(x = Time, y = Temperature)) + 
  geom_point(data = NoAttacks, aes(y =avgtemp ), size = 7, alpha = 1, shape = "-") +
  facet_grid(cols = vars(Tag), rows = vars(`Anzahl Raynaud Attacken`))+
  labs(x = "Uhrzeit", y = "Körpertemperatur") +
  theme_bw()
...
Attacks %>% ungroup %>%
    ggplot(aes(x = Time, y = Temperature)) + 
  geom_point(data = Attacks, aes(y =avgtemp ), size = 2, alpha = 0.3, shape = 16) +
  facet_grid(cols = vars(Tag), rows = vars(`Anzahl Raynaud Attacken`))+
  labs(x = "Uhrzeit", y = "Körpertemperatur") +
  theme_bw()
```
